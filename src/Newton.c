...
/* -------------------------------------------------------------------*/
void Newton(CCTK_POINTER_TO_CONST const cctkGH,
            int const nvar, int const n1, int const n2, int const n3,
            derivs v,
...
#pragma omp parallel for
    ...
    if (verbose == 1)
    {
      ...
      printf("bare mass: mp=%g \t mm=%g \t mt=%g \n", (double)par_m_plus, (double)par_m_minus, (double)par_m_third);
    }
