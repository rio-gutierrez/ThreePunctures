/* ThreePunctures:  File  "FuncAndJacobian.c"*/
        ...
        derivs u)
{
  ...
  CCTK_REAL psi, psi2, psi4, psi7, r_plus, r_minus, r_third;
  FILE *debugfile = NULL;
    ...
    for (j = 0; j < n2; j++)
    {
      for (k = 0; k < n3; k++)
      {
        ...
        if (debugfile && (k==0))
        {
          ...
          psi = 1.+
                .5 * par_m_plus  / r_plus +
                .5 * par_m_minus / r_minus +
                .5 * par_m_third / r_third +
                U.d0[0];
          psi2 = psi * psi;
          psi4 = psi2 * psi2;
          psi7 = psi * psi2 * psi4;
...
/*                      0.125 * BY_KKofxyz (x, y, z) / psi7 +*/
             ...
             (double)((U.d11[0] +
                       U.d22[0] +
                       U.d33[0])*FAC),
             (double)(-(2. * Pi / psi2/psi * sources[indx]) * FAC),
             (double)sources[indx]
             /*(double)F[indx]*/
             );
        }
...
{				/* Calculates rows of the vector 'J(FD)*dv'.*/
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    ...
    dV0 = dv.d0[iccc];
    dV1 = .5 * ga * (dv.d0[ipcc] - dv.d0[imcc]);
    dV2 = .5 * gb * (dv.d0[icpc] - dv.d0[icmc]);
    dV3 = .5 * gp * (dv.d0[iccp] - dv.d0[iccm]);
    ...
    dV12 =
      .25 * gagb * (dv.d0[ippc] - dv.d0[ipmc] + dv.d0[immc] - dv.d0[impc]);
    dV13 =
      .25 * gagp * (dv.d0[ipcp] - dv.d0[imcp] + dv.d0[imcm] - dv.d0[ipcm]);
    dV23 =
      .25 * gbgp * (dv.d0[icpp] - dv.d0[icpm] + dv.d0[icmm] - dv.d0[icmp]);
